...
forward only x steps. Return the current url after forwarding 
...
public:    
    ...
    void visit(string url) {
        currentLoc++;
        pages.resize(currentLoc);
        pages.push_back(url);
    }
    
    string back(int steps) {
        if (currentLoc-steps < 0) {
            currentLoc = 0;
        } else {
            currentLoc -= steps;
        }
        return pages[currentLoc];
    }
    
    string forward(int steps) {
        if (currentLoc+steps >= pages.size()) {
            currentLoc = pages.size()-1;
        } else {
            currentLoc += steps;
        }
        return pages[currentLoc];
