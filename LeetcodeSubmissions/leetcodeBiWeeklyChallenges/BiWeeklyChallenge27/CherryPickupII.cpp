...
(i+1, j+1).
When any robot is passing through a cell, It picks it up all 
cherries, and the cell becomes an empty cell (0).
...
/* Discussion - 
Idea
This is dp solution. At each step, we move both robot1 and robot2 
to next row, and with all possibles columns (j-1, j, j+1). Please 
keep in mind that if 2 robots go the same cell, we only collect 
cherries once.
*/
...
public:
    ...
    int dfs(vector<vector<int>>& grid, int m, int n, int r, int c1, int c2) {
        ...
        int ans = 0;
        for (int i = -1; i <= 1; i++) { // for robot 1
            for (int j = -1; j <= 1; j++) { // for robot 2
